...
- 날씨 탭: 한국 도시는 KMA 우선, 해외 도시는 OpenWeather 기반 현재 날씨와 5일 예보 표시. 기본 도시는 `서울`이며 강수확률과 현재/예상 강수량을 함께 표시
...
요청 도시명은 서버에서 먼저 정규화합니다. 한국 주요 도시는 KMA 격자 좌표로 우선 조회하고, 해외 도시 또는 KMA 실패 fallback은 `Madrid,ES` 같은 OpenWeather 검색어로 매핑한 뒤 OpenWeather Geocoding API로 위경도를 얻어 현재 날씨와 5일 예보를 조회합니다.

도시를 찾지 못하면 `CITY_NOT_FOUND` 코드를 반환하며, 사용자 UI에는 OpenWeather 내부 오류나 환경 변수명을 노출하지 않습니다.

날씨 UI의 `강수량`은 현재 관측 강수량이 있으면 `nmm 현재`로 표시하고, 현재 관측값이 0이지만 오늘 예보 강수량이 있으면 `nmm 예상`으로 표시합니다. 5일 예보 카드에는 일별 강수확률과 일별 예상 강수량을 함께 표시합니다.
...
- KMA 강수값은 `RN1`(최근 1시간), `PCP`(예보 강수량), `SNO`(예보 적설)를 사용하며 `강수없음`, `1.0mm 미만`, `30.0~50.0mm` 같은 문자열 표현을 숫자로 정규화
